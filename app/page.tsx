...
const emptyForm = { date:new Date().toISOString().slice(0,10), code:"SNV", name:"", colors:"Hitam, Navy", sourceId:"", destination:"", officer:"", note:"" };
...
export default function Home(){
  ...
  function openRecord(){
    ...
    setMatrix(first?.variants.map(x=>({...x}))??(model?buildMatrix(model):[])); setModal("record");
  }
  function selectSource(id:string){
    const sourceStage=stageInfo[active].source; const source=(data.records[sourceStage??""]??[]).find(x=>x.id===id);
    setForm({...form,sourceId:id,code:source?.modelCode??form.code}); if(source)setMatrix(source.variants.map(x=>({...x})));
  }
  ...
  async function addMaster(e:FormEvent){
    e.preventDefault(); const colors=form.colors.split(",").map(x=>x.trim()).filter(Boolean); const model:Model={code:form.code.toUpperCase(),name:form.name,colors,sizes:SIZES,active:true};
    if(data.models.some(x=>x.code===model.code)){flash("Kode jaket sudah digunakan.");return}
    await persist({...data,models:[...data.models,model]});setModal(null);flash(`${model.name} berhasil ditambahkan.`);
  }
  async function addRecord(e:FormEvent){
    ...
    const record:RecordRow={id:code(info.prefix,model.code,(data.records[active]??[]).length,form.date),stage:active,date:form.date,modelCode:model.code,modelName:model.name,sourceId:form.sourceId,variants:matrix.filter(x=>x.qty>0),total:sum(matrix),status:active==="Quality Control"?"Menunggu hasil QC":"Aktif",destination:form.destination,note:form.note};
  ...
  return <main className="app-shell">
    ...
    <section className="app-main"><header className="topbar"><label><span>⌕</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Cari PO, bundle, surat jalan..." /></label><span className={`sync ${saving?"busy":""}`}>{saving?"● Menyimpan":"✓ Tersimpan"}</span><div className="top-avatar">AR</div></header><div className="workspace">
      {!loaded?<div className="loading">Menyiapkan data produksi…</div>:<>
        ...
        {active==="Master Jaket"&&<Master data={data} onAdd={()=>{setForm(emptyForm);setModal("master")}}/>}
        {stages.includes(active)&&<StagePage active={active} rows={current} sourceCount={(data.records[stageInfo[active].source??""]??[]).length} onAdd={openRecord}/>}
    ...
    {modal==="master"&&<div className="overlay"><form className="form-modal" onSubmit={addMaster}><button className="close" type="button" onClick={()=>setModal(null)}>×</button><p className="overline">MASTER DATA</p><h2>Tambah Model Jaket</h2><div className="field-grid"><label>Kode jaket<input required maxLength={5} value={form.code} onChange={e=>setForm({...form,code:e.target.value.toUpperCase()})}/></label><label>Nama model<input required placeholder="Contoh: Orion" value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></label><label className="full">Daftar warna <small>(pisahkan dengan koma)</small><input required value={form.colors} onChange={e=>setForm({...form,colors:e.target.value})}/></label></div><div className="size-preview"><small>Ukuran standar</small>{SIZES.map(x=><b key={x}>{x}</b>)}</div><div className="form-actions"><button type="button" onClick={()=>setModal(null)}>Batal</button><button className="primary">Simpan model</button></div></form></div>}

    {modal==="record"&&<div className="overlay"><form className="form-modal matrix-modal" onSubmit={addRecord}><button className="close" type="button" onClick={()=>setModal(null)}>×</button><p className="overline">{active.toUpperCase()}</p><h2>{active==="Order Produksi"?"Buat PO Produksi":`Catat ${stageInfo[active].title}`}</h2><div className="field-grid"><label>Tanggal<input required type="date" value={form.date} onChange={e=>setForm({...form,date:e.target.value})}/></label>{active==="Order Produksi"?<label>Model jaket<select value={form.code} onChange={e=>{const m=data.models.find(x=>x.code===e.target.value)!;setForm({...form,code:e.target.value});setMatrix(buildMatrix(m));}}>{data.models.filter(x=>x.active).map(x=><option key={x.code} value={x.code}>{x.code} — {x.name}</option>)}</select></label>:<label className="full">Sumber dari {stageInfo[active].source}<select required value={form.sourceId} onChange={e=>selectSource(e.target.value)}><option value="">Pilih transaksi sumber</option>{(data.records[stageInfo[active].source??""]??[]).map(x=><option key={x.id} value={x.id}>{x.id} · {x.modelName} · {x.total} unit</option>)}</select></label>}{stageInfo[active].move&&<><label>Tujuan / lokasi<input required placeholder="Contoh: Konveksi Bintang" value={form.destination} onChange={e=>setForm({...form,destination:e.target.value})}/></label><label>Petugas<input required placeholder="Nama petugas" value={form.officer} onChange={e=>setForm({...form,officer:e.target.value})}/></label></>}</div>
...
function Master({data,onAdd}:{data:AppData;onAdd:()=>void}){return <><div className="page-title"><div><p className="overline">MASTER DATA</p><h1>Model Jaket</h1><span>Model, warna, dan ukuran yang dapat dipakai untuk PO.</span></div><button className="primary" onClick={onAdd}>＋ Tambah jaket</button></div><div className="model-grid">{data.models.map(m=><article key={m.code}><div><span>{m.code}</span><em>Aktif</em></div><h2>{m.name}</h2><small>WARNA</small><p>{m.colors.map(c=><b key={c}>{c}</b>)}</p><small>UKURAN</small><p>{m.sizes.map(s=><b key={s}>{s}</b>)}</p><footer>Kode otomatis transaksi: <strong>{m.code}</strong></footer></article>)}</div></>}
function StagePage({active,rows,sourceCount,onAdd}:{active:string;rows:RecordRow[];sourceCount:number;onAdd:()=>void}){const info=stageInfo[active];const blocked=!!info.source&&sourceCount===0;return <><div className="page-title"><div><p className="overline">PROSES TERHUBUNG</p><h1>{info.title}</h1><span>{info.desc}</span></div><button className="primary" disabled={blocked} onClick={onAdd}>＋ {active==="Order Produksi"?"Buat PO":"Catat proses"}</button></div>{blocked&&<div className="dependency"><b>Proses sebelumnya belum tersedia</b><span>Buat data di modul {info.source} terlebih dahulu agar rincian warna dan ukuran dapat diteruskan.</span></div>}<section className="panel table-panel">{rows.length===0?<Empty title={`Belum ada ${active.toLowerCase()}`} text={blocked?`Menunggu data dari ${info.source}.`:"Mulai transaksi pertama; kode akan dibuat otomatis."}/>:<div className="scroll"><table><thead><tr><th>KODE OTOMATIS</th><th>TANGGAL</th><th>SUMBER</th><th>MODEL</th><th>RINCIAN</th><th>TOTAL</th><th>STATUS</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td><b>{r.id}</b></td><td>{r.date}</td><td>{r.sourceId||"Master Jaket"}</td><td><b>{r.modelName}</b><small>{r.modelCode}</small></td><td><small>{r.variants.map(v=>`${v.color} ${v.size}: ${v.qty}`).join(" · ")}</small></td><td><b>{r.total}</b> unit</td><td><span className="status">{r.status}</span></td></tr>)}</tbody></table></div>}</section></>}
